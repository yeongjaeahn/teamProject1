...
// indexedDB에 연결하고, 연결 성공 시 데이터베이스 객체를 Promise로 감싸 반환.
const openDatabase = () => {
...
// indexedDB에 저장된 값읋 가져옴
const getFromDB = async (storeName, key = "") => {
  // database 변수가 아직 초기화 전이면, openDatabase 함수를 실행하여 데이터베이스 객체를 할당함.
  if (!database) {
  ...
  const data = new Promise((resolve, reject) => {
    // key가 주어졌다면 key에 해당하는 특정 아이템만, key가 없다면 모든 아이템 get
...
const addToDB = async (storeName, entry, key = "") => {
  ...
  const result = new Promise((resolve, reject) => {
    // key가 있다면 해당 key로 db에 추가, 없으면 기본 설정대로 autoIncrement(1,2,3.. 순서로) key를 설정하여 추가함.
