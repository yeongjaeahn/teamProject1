...
userRouter.patch("/users/:_Id", loginRequired, async function (req, res, next) {
  ...
  const user = await userModel.updateByEmail({
    userEmail: userEmail,
    update: { fullName: name, phoneNumber: phoneNumber, address: address },
  });
  if (!user) {
...
userRouter.delete(
  ...
  async function (req, res, next) {
    const userEmail = req.params._Id;
    const deletedUser = await userModel.deleteByEmail({ email: userEmail });
    if (!deletedUser) {
      next(new Error("해당 아이디의 유저가 없습니다."));
      return;
    }
    res.send({ 탙퇴이메일: userEmail });
  }
);
